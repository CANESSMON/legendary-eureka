...
def send_reset_password_email(to_email: str, otp_code: str):
    """
    Sends a password reset OTP email to the specified recipient using SMTP.
    Also prints it to the console as a fallback.
    """
    # Print to console as fallback/logging
    print(f"\n========================================\n[RESET DEBUG] Sent to: {to_email}\n[RESET DEBUG] Purpose: PASSWORD RESET\n[RESET DEBUG] OTP Code: {otp_code}\n========================================\n")
    
    subject = "Your JobPortal Password Reset Code"
    ...
    <html>
        <body style="font-family: Arial, sans-serif; margin: 0; padding: 20px; background-color: #f4f4f7; color: #333333;">
            <div style="max-width: 500px; margin: 0 auto; background: #ffffff; padding: 30px; border-radius: 10px; box-shadow: 0 4px 10px rgba(0,0,0,0.05);">
                <div style="text-align: center; margin-bottom: 20px;">
                    <h2 style="color: #0F172A; margin: 0;">JobPortal India</h2>
                    <p style="font-size: 14px; color: #64748B; margin-top: 5px;">Password Reset Request</p>
                </div>
                <hr style="border: 0; border-top: 1px solid #E2E8F0; margin-bottom: 25px;">
                <p style="font-size: 16px; line-height: 1.5;">Hello,</p>
                <p style="font-size: 16px; line-height: 1.5;">We received a request to reset your password. Use the code below to set a new password:</p>
                <div style="text-align: center; margin: 30px 0;">
                    <span style="font-size: 32px; font-weight: bold; letter-spacing: 5px; color: #DC2626; background: #FEF2F2; padding: 10px 24px; border-radius: 8px; border: 1px dashed #FECACA; display: inline-block;">
                        {otp_code}
                    </span>
                </div>
                <p style="font-size: 14px; color: #64748B; line-height: 1.5;">This code is valid for <strong>15 minutes</strong>. If you did not request a password reset, please ignore this email.</p>
                <hr style="border: 0; border-top: 1px solid #E2E8F0; margin-top: 25px; margin-bottom: 20px;">
                <p style="font-size: 12px; text-align: center; color: #94A3B8; margin: 0;">
                    &copy; 2026 JobPortal India. All rights reserved.
                </p>
